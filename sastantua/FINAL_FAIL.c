...
/*   Updated: 2016/08/07 18:06:30 by mgonon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_door(int space_nb, int floor_height, int star_nb, int last)
{
	int door_char_nb;
	int regular_lign;
	int *pt_regular_lign;
	...
	*pt_regular_lign = &regular_lign;
	...
	if (floor_height % 2 == 0)
		ft_print_lign(space_nb, star_nb, door_char_nb, *pt_regular_lign);
	else
		ft_print_lign(space_nb, star_nb, door_char_nb, *pt_regular_lign - 1);
}
...
void	ft_print_sastantua(int space_nb, int sas_height)
{
	...
	while (tab[0] <= sas_height)
	{
		...
		space_nb -= tab[3] / 2;
		tab[4] = tab[1];
		ft_print_door(space_nb, tab[1], tab[2], 0);
		while (tab[4] > 0)
		{
			tab[2] += 2;
			space_nb -= 1;
			tab[4]--;
			if (tab[0] == sas_height)
				ft_print_door(space_nb, tab[1], tab[2], 1);
			else
				ft_print_door(space_nb, tab[1], tab[2], 0);
		}
		if (tab[0] % 2 == 1)
